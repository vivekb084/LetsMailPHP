...
if(isset($_SESSION['login_user']))
{
   ...
   $_SESSION['nooffile']=0;
    ...
    ?>
<?php $sql="select * from messagefile order by Messageid DESC LIMIT 1";
...
$results=mysql_query($sql);
       if(mysql_num_rows($results)==0)
            ...
            $sq2="select * from sent order by Messageid DESC LIMIT 1";
            $rs2= mysql_query($sq2);
        ...
        $s=(string)filter_input(INPUT_GET, 'mi');
        $bo=(string)filter_input(INPUT_GET, 'Box');
        $us=$_SESSION['login_user'];
        $sqqu="select * from $bo where messageid='$s' and toemail='$us'";
        $t=mysql_query($sqqu)or die(mysql_error());
        while($v=  mysql_fetch_array($t))
        {
...
<div class="form_settings">
    ...
    <form method="post" action="sendmail.php" >                                                       
        <p><span>To</span><input class="contact" type="text" name="tos" value="<?php echo $v['fromemail'];?>" /></p>
        <p><span>Subject</span><input class="contact" type="text" name="subject" value="Reply of <?php echo $v['subject'];?>" /></p>
        <p><span>Message</span><textarea class="contact textarea" rows="12" cols="50" name="message" ><?php $myfiles = fopen('Message/'.$s.'.txt'  , "r");
echo fread($myfiles,filesize('Message/'.$s.'.txt'  ));
fclose($myfile);
            ?></textarea></p>
...
var settings = {
	...
	method: "POST",
	allowedTypes:"jpg,png,gif,doc,pdf,zip,txt,docx,zip",
	fileName: "myfile",
	multiple: true,
